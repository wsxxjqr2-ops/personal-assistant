...
export function registerMetadataTools(
  ...
  function getClient(): { client: RedmineClient } {
  ...
  server.tool(
    'list_versions',
    '查询项目的目标版本 (fixed_version / 迭代排期 / 里程碑列表)。创建或筛选任务时非常有用。',
    {
      project_id: z
  ...
  server.tool(
    'list_categories',
    '查询项目的模块分类列表 (issue categories，如云控、前端、设计、支持等)。创建任务时选择合适类别。',
    {
      project_id: z
        .union([z.number(), z.string()])
        .optional()
  ...
  server.tool(
    'list_trackers_and_statuses',
    '查询协作平台支持的跟踪类型 (Trackers，如功能、支持、缺陷) 和任务状态列表 (Statuses，如进行中、已解决)。',
    {},
    async () => {
      try {
        const { client } = getClient();
