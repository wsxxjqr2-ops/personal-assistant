...
import { UserSession, RedmineUser } from './types.js';
...
export class AuthManager {
  ...
  public async loginWithPassword(
    ...
    const data = (await res.json()) as { user: RedmineUser };
  ...
  private loadFromFile(): UserSession | null {
    try {
      if (fs.existsSync(CONFIG_FILE)) {
  ...
  private saveToFile(session: UserSession): void {
    try {
