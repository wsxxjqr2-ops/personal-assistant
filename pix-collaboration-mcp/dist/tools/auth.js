...
export function registerAuthTools(server, authManager, getSessionId) {
    server.tool('login', '登录 PIX/RUBYLOFT 协作平台 (rd.pixmoving.city)。支持通过协作平台日常账号密码登录（自动获取 API Key），或直接提供 API Key。登录成功后凭据将安全保存在本地，无需重复登录。', {
        ...
        try {
            ...
            if (username && password) {
                const session = await authManager.loginWithPassword(username, password, sessionId, base_url);
                return {
                    content: [
                        {
                            type: 'text',
    ...
    server.tool('whoami', '检查当前协作平台的登录状态和个人信息（包括姓名、用户名、用户 ID、平台地址等）。', {}, async () => {
        const sessionId = getSessionId ? getSessionId() : undefined;
    ...
    server.tool('logout', '退出当前登录状态，清除保存在本地的凭据缓存。', {}, async () => {
        const sessionId = getSessionId ? getSessionId() : undefined;
