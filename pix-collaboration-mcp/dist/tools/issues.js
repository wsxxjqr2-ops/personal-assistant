...
export function registerIssueTools(server, authManager, getSessionId) {
    function getClient() {
    ...
    server.tool('query_my_issues', '查询指派给当前登录人的任务列表。支持按状态（open 开启中 / * 全部 / closed 已关闭）、目标版本、模块分类或关键字筛选。', {
        status: z
            .enum(['open', '*', 'closed'])
            .optional()
            .default('open')
        ...
        try {
            ...
            for (const it of res.issues) {
                ...
                lines.push(`| ${idLink} | ${tracker} | ${subject} | ${st} | ${progress} | ${due} | ${ver} | ${cat} |`);
            }
    ...
    server.tool('search_issues', '多条件搜索协作平台的任务（不仅限于指派给自己的任务）。', {
    ...
    server.tool('get_issue_detail', '获取单个任务的详细信息，包含任务描述、工时预估、历史变更记录与备注。', {
    ...
    server.tool('create_issue', '在协作平台创建新的任务。注意：平台强制要求提供“计划完成日期 (due_date)”和“预期时间 (estimated_hours)”。', {
    ...
    server.tool('update_issue', '更新任务状态、完成进度、或追加进展备注。', {
        ...
        try {
            ...
            const updates = [];
            if (done_ratio !== undefined)
                updates.push(`进度更新为 ${done_ratio}%`);
            if (status_id !== undefined)
                updates.push(`状态 ID 变更为 ${status_id}`);
            if (due_date !== undefined)
                updates.push(`到期日变更为 ${due_date}`);
            if (notes)
                updates.push(`追加备注: "${notes}"`);
            return {
                content: [
                    {
                        type: 'text',
